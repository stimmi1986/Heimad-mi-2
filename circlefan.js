...
function init() {
    ...
    canvas = document.getElementById( "gl-circlefan" );
    ...
    var segOut    = document.getElementById("segVal");
    segOut.textContent = segSlider.value;

    segSlider.addEventListener("input", function () {
        numCirclePoints = parseInt(segSlider.value, 10);
        segOut.textContent = numCirclePoints;
...
function createCirclePoints( cent, rad, k )
{
    ...
    for( i=k; i>=0; i-- ) {
    	a = i*dAngle;
    	var px = rad * Math.sin(a) + cent[0];
    	var py = rad * Math.cos(a) + cent[1];
